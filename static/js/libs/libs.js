...
function getValues(){
  ...
  Array.from(document.getElementsByClassName("user-values-inputs-select")).forEach((select)=>{
    let index=Array.from(document.getElementsByClassName("user-values-inputs-select")).indexOf(select)
    values[index]*=Array.from(document.getElementsByClassName("user-values-inputs-select"))[index].value
  });
  return values
}
...
function loadData(){
  let data={}
  data.current_values=[50000,50000,50000,0,30000,1000,1000,1000,1000,0,0]
  if(localStorage.getItem("current_values")!=null){
    data.current_values=JSON.parse(localStorage.getItem("current_values").toString())
  }
  data.history=[]
  if(localStorage.getItem("history")!=null){
